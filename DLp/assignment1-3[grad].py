...
def gradient_check(x, theta, epsilon = 1e-7):
    ...
    denom = np.linalg.norm(gradapprox + grad)
    diff = num / denom
    return diff
...
def gradient_check_n(parameters, gradients, X, Y, epsilon = 1e-7):
    ...
    for i in range(num_parameters):
        ...
        J_plus[i], _ = forward_propagation_n(X, Y, vector_to_dictionary(plus_copy))
        # Compute J_minus[i]. Inputs: "parameters_values, epsilon". Output = "J_minus[i]".
        minus_copy = parameters_values.copy()
        minus_copy[i] = minus_copy[i] - epsilon
        J_minus[i], _ = forward_propagation_n(X, Y, vector_to_dictionary(minus_copy))
